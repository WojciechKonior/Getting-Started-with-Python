...
int main()
{
    ...
    char buffer[BUFSIZ];
    char sendstr[BUFSIZ];
    const int backlog = 10; // also max connection
    ...
    if(serverfd==-1) {perror("failed"); exit(1);}
    ...
    FD_ZERO(&main_fd); // initailize file descriptor set
    FD_SET(serverfd, &main_fd); // set the server's file descriptor
    
    // endles loop to process the connections
    done = FALSE;
    while(!done)
    {
        // backup the main file desciptor set into a read set for processing
        read_fd = main_fd;

        // scan connections for any activity
        r = select(max_connect + 1, &read_fd, NULL, NULL, 0);
        if(r==-1){perror("failed"); exit(1);}

        // lop to check for active connections
        for(fd=1; fd<=max_connect; fd++)
        {
            // fileter only active or new clients
            if(FD_ISSET(fd, &read_fd))
            {
                ...
                if(fd==serverfd)
                {
                    ...
                    // add new client socket ot the file descriptor list
                    FD_SET(clientfd, &main_fd);

                    //welcome the new user: create selcome string and send
                    // welcome string "SERVER> welcome xxx.xxx.xxx.xxx to the chat server\n"
                    // "Type 'close' to disconnect; 'shutdown' to stop\n"
                    ...
                    // build th string: "SERVER> xxx.xxx.xxx.xxx has joined the server"
                ...
                else
                {
                    ...
                    if(r<1)
                    {
                        ...
                        // build the string
                        // "SERVER> xxx.xxx.xxx.xxx disconnected"
                        strcpy(buffer, "SERVER> ");
                        strcat(buffer, connection[fd]);
                        strcat(buffer, " disconnected\n");

                        //loop through all connections
                        //(not the server) to send the string
                        for(x=serverfd+1; x<max_connect; x++)
                        {
                            if(FD_ISSET(x, &main_fd))
                            {
                    ...
                    else
                    {
                        ...
                        //first check to see wheater the "shutdown\n" string was sent
                        if(strcmp(buffer, "shutdown\n")==0)
                        {
                            done = TRUE; // if so, set the loop's termiating condition
                        }
